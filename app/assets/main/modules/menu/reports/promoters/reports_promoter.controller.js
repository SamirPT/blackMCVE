/**
 * Created by EGusev on 08.07.2016.
 */
(function () {
    'use strict';

    angular.module('mrblack.main').controller('ReportsPromoterCtrl', ['$scope', 'ReportsService', '$state', 'Toastr', 'VenueService', 'UtilService', function ($scope, ReportsService, $state, Toastr, VenueService, UtilService) {
        ...
        $scope.getReportPDF = function (dateFrom, dateTo, reportType) {
            ...
            ReportsService.getPromotersReportPDF({dateFrom: dateFrom, dateTo: dateTo, venueId: $state.params.id, reportType: reportType}).then(
        ...
        $scope.getReport = function(dateFrom, dateTo, reportType) {
            ...
            ReportsService.getPromotersReport({dateFrom: dateFrom, dateTo: dateTo, venueId: $state.params.id, reportType: reportType}).then(
                function successCallback(response) {
                    if (reportType === REPORT_TYPE_DAILY) {
                        $scope.promoters = response.promoters;
                    ...
                    if (reportType === REPORT_TYPE_WEEKLY) {
                        $scope.promotersWeekly = response.promoters;
                    ...
                    if (reportType === REPORT_TYPE_MONTHLY) {
                        $scope.promotersMonthly = response.promoters;
                    ...
                    if (reportType === REPORT_TYPE_INTERVAL) {
                        $scope.promotersInterval = response.promoters;
        ...
        $scope.clearWeekFilter = function () {
            $scope.selectedPromoterWeek = '';
        };

        $scope.clearMonthFilter = function () {
            $scope.selectedPromoterMonth = '';
        };

        $scope.clearIntervalFilter = function () {
            $scope.selectedPromoterInterval = '';
        };
